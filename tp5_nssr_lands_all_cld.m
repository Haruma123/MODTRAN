...
suff=repmat([2 1],7,1);
mkdir('17land_tp5_cld_annual_aver\');
for k=1:17
    p3=textread(['E:\学术\MATLAB\NSSR\MODTRAN part\brdf_modis_read\3brdf_17lands_annual_aver\',fns{k}]);
    p3=[wl,p3,suff];
    mkdir(['17land_tp5_cld_annual_aver\',nms{k}]);
    for r=1:length(RSA)
        for s=1:length(SZA)
            for v=1:length(VZA)
                tp5=['17land_tp5_cld_annual_aver\',nms{k},'\',nms{k},'_',num2str(r),num2str(s),num2str(v),'.tp5'];
                fid=fopen(tp5,'w');
                ...
                for i=1:length(MODEL)
                    for j=1:length(IHAZE)
                        ...
                        fprintf(fid,'    0    0    3');
                        fprintf(fid,'%5d',4);% ICLD，0晴天，1.2.3.4.5云天时card2a要设置
                        fprintf(fid,'    0');
                        fprintf(fid,'%10.5f',VIS(j));
                        fprintf(fid,'   0.00000   0.00000   0.00000   0.00200\n');%最后一个0.0000是DEM，可以进行改变
                        %card2a
                        fprintf(fid,'  -9.000  -9.000  -9.000  -9  -9  -9.000  -9.000  -9.000  -9.000  -9.000  -9.000\n');
...
%% 获取tp5list
for k=1:17
    files=dir(['17land_tp5_cld_annual_aver\',nms{k},'\','*.tp5']);
    for i=1:length(files)
        list{i}=files(i).name;
    end
    
    %[filepath,name,ext] = fileparts(list(1));
    fid=fopen(['17land_tp5_cld_annual_aver\',nms{k},'\','mod5root.in'],'w');
    fprintf(fid,'%s\r\n',list{:});
    %fprintf(fid,'%s\n',string(list));
    fclose(fid);
end